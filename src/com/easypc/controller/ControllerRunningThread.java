...
public class ControllerRunningThread implements Runnable {
	...
	public void run() {
		while (isRunning) {
			// TODO: fetch the opCode from the RAM not from the data byte
			// pointer (!)
			// data=ram.read(cpu.getRegister(19), 2);
			// cpu.executeOpCode((data.get(0)&0xF0)>>4, (data.get(0)&0x0F),
			// data.get(1)&0xF0>>4, data.get(1)&0x0F);
			int PC = cpu.getRegister(19);
			ArrayList<Integer> opCode = ram.read(PC, 2);
			int temp = opCode.get(1);
			temp = temp & 0xF0;
			temp = temp >> 4;
			cpu.executeOpCode((opCode.get(0) & 0xF0) >> 4,
					(opCode.get(0) & 0x0F), (opCode.get(1) & 0xF0) >> 4,
					opCode.get(1) & 0x0F);
			if (_main.DEBUG) {
